    ...
    return (
      <div className="absolute inset-0 flex items-center justify-center bg-gradient-to-br from-cinematic-bg via-cinematic-panel to-cinematic-bg">
        ...
        <div className="relative flex flex-col items-center gap-3 p-6">
          ...
          <span className="font-mono text-micro tracking-wide text-brand-ember/30 text-center max-w-[120px] leading-relaxed">
            {alt}
          </span>
        </div>
      </div>
    ...
    <div className="w-full overflow-hidden border-y border-border bg-primary/[0.03] py-2.5">
      ...
      >
        {TICKER_ITEMS.map((item, i) => (
          <span
            key={i}
            className="flex items-center gap-5 font-sans text-xs tracking-wide text-primary font-bold shrink-0"
          >
  ...
  return (
    <div className="absolute inset-0 z-0 pointer-events-none flex flex-col justify-center overflow-hidden">
      ...
      <div className="hidden md:flex flex-col gap-3 mt-auto absolute bottom-24 left-6 md:left-8 lg:left-10 right-6 md:right-8 lg:right-10 opacity-30 group-hover:opacity-100 transition-opacity duration-700">
        <div className="flex items-center gap-4 text-micro font-sans font-medium tracking-wide text-primary">
        ...
        <div className="flex items-end gap-[3px] h-8">
          ...
          <div className="h-8 border-l border-dashed border-primary/30 ml-3 pl-3 flex flex-col justify-end pb-0.5">
            <span className="text-micro font-mono text-primary/50 tabular-nums tracking-wide leading-none">
    ...
    >
      <Link href={`/stories/${story.slug}`} className="flex flex-col md:flex-row relative">
        ...
        <div className="relative w-full md:w-[380px] lg:w-[440px] shrink-0 aspect-[3/4] overflow-hidden bg-muted border-b md:border-b-0 md:border-r border-border z-10">
          ...
          {/* Featured badge */}
          <span className="absolute top-4 left-4 z-10 font-mono text-nano tracking-wide text-primary-foreground bg-primary px-3 py-1.5 shadow-sm">
            Featured
          </span>
        </div>
        ...
        <div className="flex flex-col justify-between flex-1 p-6 md:p-8 lg:p-10 gap-4 relative">
          ...
          <div className="space-y-4 relative z-10">
            ...
            <div className="flex items-center gap-3">
              ...
              />
              <span className="text-fine font-sans font-medium tracking-wide text-primary font-bold">
                {getCategoryLabel(story.category)}
              </span>
              <span className="text-muted-foreground/30">·</span>
              <span className="font-mono text-nano tracking-wide text-muted-foreground flex items-center gap-1">
            ...
            <div className="flex items-center gap-4">
              <div className="flex flex-wrap gap-2">
                {story.tags.slice(0, 3).map((tag) => (
                  <span key={tag} className="text-nano font-sans font-medium tracking-wide text-foreground bg-secondary border border-border px-2 py-1">
                    {tag}
                  </span>
                ))}
              </div>
              {story.chapterCount !== undefined && story.chapterCount > 0 && (
                <span className="text-nano font-sans font-medium tracking-wide text-muted-foreground flex items-center gap-1">
                  <BookOpen className="size-3 text-primary/60" />
                  {story.chapterCount} {story.chapterCount === 1 ? "chapter" : "chapters"}
                </span>
              )}
            </div>
            <span className="flex items-center gap-2 font-sans text-xs tracking-wide text-primary font-bold group-hover:gap-3 transition-all">
              Read
    ...
    >
      <Link href={`/stories/${story.slug}`} className="flex flex-col h-full bg-border gap-px">
        ...
        <div className="relative w-full aspect-[3/4] overflow-hidden bg-muted shrink-0">
          ...
          {/* Language badge */}
          <span className="absolute top-2 left-2 z-10 font-mono text-nano tracking-wide bg-background/90 border border-border px-2 py-1 flex items-center gap-1 backdrop-blur-sm">
            <Globe className="size-2.5 text-primary" />
            {getLanguageLabel(story.language)}
          </span>
          ...
          {/* View count */}
          {(story.viewCount ?? 0) > 0 && (
            <span className="absolute top-2 right-2 z-10 font-mono text-micro tracking-wide bg-background/90 border border-border px-2 py-1 flex items-center gap-1 backdrop-blur-sm text-muted-foreground">
...
function ListRow({ story, index }: { story: Story; index: number }) {
  return (
    ...
    >
      ...
      >
        {/* Index */}
        <span className="font-sans text-xs text-muted-foreground w-6 shrink-0 tabular-nums">
        ...
        <div className="flex-1 min-w-0">
          ...
          <p className="font-mono text-nano tracking-wide text-muted-foreground mt-0.5">
        ...
        <div className="hidden md:flex items-center gap-2 shrink-0">
          {story.tags.slice(0, 2).map((t) => (
            <span key={t} className="text-micro font-sans font-medium tracking-wide bg-secondary border border-border px-2 py-0.5">
              {t}
            </span>
          ))}
        </div>
              ...
              >
                <div className="py-4 flex flex-col md:flex-row gap-6 border-b border-border">
                  ...
                  <div className="flex-1">
                    <span className="text-nano font-sans font-medium tracking-wide text-muted-foreground block mb-2">Category</span>
                    <div className="flex flex-wrap gap-1.5">
                      {CATEGORIES.map((cat) => (
                        <button
                          ...
                          className={cn(
                            "text-nano font-sans font-medium tracking-wide px-2.5 py-1 border transition-all",
                  ...
                  <div className="shrink-0">
                    <span className="text-nano font-sans font-medium tracking-wide text-muted-foreground block mb-2">Language</span>
                    <div className="flex flex-wrap gap-1.5">
                      {LANGUAGES.map((lang) => (
                        <button
                          key={lang.value}
                          ...
                          className={cn(
                            "text-nano font-sans font-medium tracking-wide px-2.5 py-1 border flex items-center gap-1.5 transition-all",
                  ...
                  {(activeCategory || activeLanguage) && (
                    <div className="flex items-end">
                      <button
                        ...
                        className="text-nano font-sans font-medium tracking-wide text-destructive flex items-center gap-1.5 hover:underline"
                      >
                        <X className="size-3" />
                        Clear
                      </button>
