...
export function DroppableColumn({ id, title, children }: DroppableColumnProps) {
  ...
  return (
    <div className="flex w-80 shrink-0 flex-col rounded-xl bg-muted/30">
      ...
      <div
        ref={setNodeRef}
        className={cn("flex-1 rounded-xl p-3 transition-colors", isOver ? "bg-muted/50" : "bg-transparent")}
      >
        {children}
      </div>
    </div>
