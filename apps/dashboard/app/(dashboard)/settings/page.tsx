...
function RoleBadge({ role }: { role: string }) {
  ...
  return (
    <div
      className={cn(
        "flex items-center justify-center gap-2 px-3 py-1.5 border-2 text-fine font-sans font-medium tracking-wide shadow-xs whitespace-nowrap",
...
function MemberRow({
  ...
  return (
    <div className="flex flex-col xl:flex-row xl:items-center justify-between p-4 md:p-6 gap-4 border-b border-border-subtle bg-cinematic-panel/40 transition-all group hover:bg-bg-surface/5">
      <div className="flex items-start md:items-center gap-4 md:gap-6 min-w-0 flex-col sm:flex-row">
        <div className="border border-border/50 p-1 bg-cinematic-bg group-hover:border-primary transition-colors shrink-0">
          <AvatarBadge
    ...
    <div className="flex flex-col min-h-full p-6 lg:p-10 lg:px-14 max-w-[1400px] mx-auto w-full space-y-16 animate-in fade-in duration-700">
      ...
      <div className="flex flex-col border-b-4 border-foreground pb-8 shrink-0 relative z-10 w-full pt-8">
        <div className="space-y-4">
          ...
          <p className="text-muted-foreground font-sans text-xs font-semibold tracking-label uppercase max-w-2xl mt-4">
      ...
      <div className="space-y-10">
        ...
        {/* Roster Sector */}
        <BrutalistCard variant="panel" padding="none" className="bg-cinematic-panel/40 overflow-hidden relative">
          <div className="absolute top-0 inset-x-0 h-px bg-gradient-to-r from-transparent via-primary/50 to-transparent" />
          ...
          <div>
            {members === undefined ? (
              <div className="animate-pulse flex flex-col p-4 gap-4">
                {[1, 2, 3].map((i) => (
                  <div key={i} className="h-24 border border-border/50 bg-cinematic-bg" />
                ))}
              </div>
            ) : members.length === 0 ? (
